...
def login():
    ...
    user_info = User.query.filter_by(login_name=login_name).first()
    if not user_info:
        return ops_renderErrJSON(msg="请输入正确的登陆用户名和密码 -1~~")
    if user_info.login_pwd != UserService.genePwd(login_pwd, user_info.login_salt):
        return ops_renderErrJSON(msg="请输入正确的登陆用户名和密码 -2~~")
    ...
    response.set_cookie(key=app.config["AUTH_COOKIE_NAME"],
                        value="%s#%s" % (UserService.geneAuthCode(user_info), user_info.id), max_age=60 * 60 * 24 * 120)
    return response
