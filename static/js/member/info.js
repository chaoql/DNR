;
var member_improve_ops = {
    init: function () {
        this.eventBind();
    },
    eventBind: function () {
        $(".do-imp").click(function () { // 前面带"$"的就是jQuery语句
            const btn_target = $(this);
            if (btn_target.hasClass("disabled")) { // 避免重复点击注册
                common_ops.alert("正在处理！请勿重复点击~~");
                return;
            }
            const gender = $(".imp-wrap input[name=gender]").val();
            const age = $(".imp-wrap input[name=age]").val();
            const occupation = $(".imp-wrap input[name=occupation]").val();
            ...
            $.ajax({
                ...
                url: common_ops.buildUrl("/member/info"),
                type: "POST",
                data: {
                    gender: gender,
                    age: age,
